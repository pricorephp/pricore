...
it('keeps entry timestamps so a rebuild produces the same archive', function () {
    $zip = new ZipArchive;
    $zip->open($this->source, ZipArchive::CREATE);
    $zip->addFromString('repo-abc/packages/billing/composer.json', 'billing');
    $zip->setMtimeName('repo-abc/packages/billing/composer.json', 1_700_000_000);
    $zip->close();

    $extractor = new ZipSubtreeExtractor;
    $extractor->extract($this->source, $this->output, 'packages/billing', 'billing-abc');

    $zip->open($this->output);
    $mtime = $zip->statName('billing-abc/composer.json')['mtime'];
    $zip->close();

    $firstBuild = sha1_file($this->output);
    unlink($this->output);
    $extractor->extract($this->source, $this->output, 'packages/billing', 'billing-abc');

    expect($mtime)->toBe(1_700_000_000)
        ->and(sha1_file($this->output))->toBe($firstBuild);
});
