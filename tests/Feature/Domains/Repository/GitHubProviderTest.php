...
function githubProvider(string $repoIdentifier = 'acme/monorepo'): GitHubProvider
{
    return new GitHubProvider($repoIdentifier, ['token' => 'github_pat_test']);
}
...
it('keeps the cached zipball intact when other requests follow the download', function () {
    $fixture = $this->directory.'/full.zip';
    ...
    Http::fake([
        ...
        'api.github.com/repos/acme/monorepo/contents/*' => Http::response([
            'type' => 'file',
            'content' => base64_encode('{"name":"acme/crm"}'),
            'encoding' => 'base64',
        ]),
    ]);
    ...
    expect($provider->downloadArchive($ref, $this->directory.'/billing.zip', 'packages/billing'))->toBeTrue()
        ->and($provider->getFileContent($ref, 'packages/crm/composer.json'))->toBe('{"name":"acme/crm"}')
        ->and($provider->downloadArchive($ref, $this->directory.'/crm.zip', 'packages/crm'))->toBeTrue()
        ->and(testZipFileNames($this->directory.'/crm.zip'))->toBe(['crm-abc123def456/composer.json']);
});
