...
it('returns null for a file missing at a ref that exists', function () {
    expect($this->provider->getFileContent($this->sha, 'packages/missing/composer.json'))->toBeNull();
});

it('throws instead of reporting a missing file when the ref cannot be read', function () {
    expect(fn () => $this->provider->getFileContent(str_repeat('f', 40), 'composer.json'))
        ->toThrow(GitProviderException::class)
        ->and(fn () => $this->provider->listDirectory(str_repeat('f', 40), 'packages'))
        ->toThrow(GitProviderException::class);
});

it('throws when the clone is gone', function () {
    File::deleteDirectory($this->directory.'/bare.git');

    expect(fn () => $this->provider->getFileContent($this->sha, 'composer.json'))
        ->toThrow(GitProviderException::class, 'The repository clone is missing.')
        ->and(fn () => $this->provider->listDirectory($this->sha, ''))
        ->toThrow(GitProviderException::class, 'The repository clone is missing.');
});
