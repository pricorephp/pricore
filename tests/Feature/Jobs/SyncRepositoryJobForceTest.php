...
it('unserializes a job queued before the force flag existed', function () {
    $payload = (new SyncRepositoryJob($this->repository))->__serialize();
    unset($payload['force']);

    $job = (new ReflectionClass(SyncRepositoryJob::class))->newInstanceWithoutConstructor();
